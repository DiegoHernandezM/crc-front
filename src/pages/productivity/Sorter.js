...
import { getSorterProductivity, getReportSorter } from '../../redux/slices/productivity';
// components
...
export default function Sorter() {
  ...
  const { sorter, isLoading, message, typeMessage, date } = useSelector((state) => state.productivity);
  ...
  const requestSearch = (searchValue) => {
    ...
    const filteredRows = sorter.filter((row) => Object.keys(row).some((field) => searchRegex.test(row[field])));
  ...
  const readExcel = (file) => {
    ...
    promise.then((d) => {
      // dispatch(loadSorterProductivity(d)).then(() => {
      //   setOpenMessage(true);
      // });
    });
  };
  ...
  function getExport() {
    return (dispatch) =>
      new Promise((resolve) => {
        resolve(dispatch(getReportSorter(selectedDate)));
      })
        .then((response) => {
  ...
  return (
    <Page title="Productividad Sorter">
