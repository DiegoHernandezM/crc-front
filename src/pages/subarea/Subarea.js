  ...
  function createSubarea(name, areaId) {
    return (dispatch) =>
      new Promise((resolve) => {
        ...
        .then((response) => {
          setMessage(response.status === 200 ? 'Subarea creada' : 'Ocurro algun error');
  ...
  function restoreSubarea() {
    return (dispatch) =>
      new Promise((resolve) => {
        ...
        .then((response) => {
          setMessage(response.status === 200 ? 'Subarea recuperada' : 'Ocurrio algun error');
          setTypeMessage(response.status === 200 ? 'success' : 'error');
          setOpenMessage(true);
          dispatch(getSubareas());
          setSubareaId(0);
  ...
  return (
    <Page title="Subareas">
      ...
      <DialogConfirm
        ...
        title={modeRestore === false ? 'Eliminar Subarea' : 'Recuperar Subarea'}
        body={modeRestore === false ? 'Desea eliminar esta subarea' : 'Desea recuperar esta subarea'}
